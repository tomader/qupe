...
function checkUserReports(user_id) {
    axios({
        method: 'post',
        url: 'https://gpqupe.000webhostapp.com/backend/check-user-reports.php',
        data: {
            user_id: user_id
        }
    })
        .then(function (response) {
            var currentUrl = 'https://gpqupe.000webhostapp.com/gp2/TheHead/one-verified-user.html';
...
$(document).ready(function () {
    axios({
        method: 'post',
        url: 'https://gpqupe.000webhostapp.com/backend/fetch-faculty.php'
    })
        .then(function (response) {
            //handle success
            console.log(response);
            $.each(response.data, function (key, value) {
                axios({
                    method: 'post',
                    url: 'https://gpqupe.000webhostapp.com/backend/fetch-deptName-from-userId.php',
                    data: {
                        user_id: value.ID
                    }
                })
